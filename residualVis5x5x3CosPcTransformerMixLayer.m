classdef residualVis5x5x3CosPcTransformerMixLayer < nnet.layer.Layer % & nnet.layer.Formattable (Optional)
    ...
    methods
        function layer = residualVis5x5x3CosPcTransformerMixLayer(numInChannels, numPatchV, numPatchH, numResChannels, name)
            ...
            layer.Wq1g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq01g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk1g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk01g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq2g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq02g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk2g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk02g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq3g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq03g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk3g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk03g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq4g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq04g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk4g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk04g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq5g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq05g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk5g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk05g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq6g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq06g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk6g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk06g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq7g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq07g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk7g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk07g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq8g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq08g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk8g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk08g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq9g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq09g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk9g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk09g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq10g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq010g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk10g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk010g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq11g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq011g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk11g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk011g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq12g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq012g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk12g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk012g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq13g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq013g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk13g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk013g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq14g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq014g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk14g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk014g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq15g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq015g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk15g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk015g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq16g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq016g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk16g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk016g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq17g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq017g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk17g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk017g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq18g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq018g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk18g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk018g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq19g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq019g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk19g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk019g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq20g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq020g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk20g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk020g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq21g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq021g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk21g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk021g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq22g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq022g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk22g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk022g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq23g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq023g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk23g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk023g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq24g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq024g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk24g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk024g = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq25g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq025g = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk25g = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk025g = zeros([layer.numPatch+layer.numResChannels, 1]);

            


            layer.Wq1b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq01b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk1b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk01b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq2b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq02b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk2b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk02b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq3b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq03b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk3b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk03b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq4b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq04b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk4b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk04b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq5b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq05b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk5b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk05b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq6b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq06b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk6b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk06b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq7b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq07b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk7b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk07b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq8b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq08b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk8b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk08b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq9b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq09b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk9b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk09b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq10b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq010b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk10b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk010b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq11b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq011b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk11b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk011b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq12b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq012b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk12b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk012b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq13b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq013b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk13b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk013b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq14b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq014b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk14b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk014b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq15b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq015b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk15b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk015b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq16b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq016b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk16b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk016b = zeros([layer.numPatch+layer.numResChannels, 1]); 

            layer.Wq17b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq017b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk17b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk017b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq18b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq018b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk18b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk018b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq19b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq019b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk19b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk019b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq20b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq020b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk20b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk020b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq21b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq021b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk21b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk021b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq22b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq022b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk22b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk022b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq23b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq023b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk23b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk023b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq24b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq024b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk24b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk024b = zeros([layer.numPatch+layer.numResChannels, 1]);

            layer.Wq25b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wq025b = zeros([layer.numPatch+layer.numResChannels, 1]);
            layer.Wk25b = bound * (2. * rand([layer.numPatch+layer.numResChannels, layer.numPatch+layer.numResChannels],'single') - 1.);
            layer.Wk025b = zeros([layer.numPatch+layer.numResChannels, 1]);
        ...
        function [Z] = predict(layer, X)
            ...
            X1g = vertcat ( X(1+cOff:cOff+layer.numPatch,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X2g = vertcat ( X(1+cOff+layer.numPatch:cOff+layer.numPatch*2,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X3g = vertcat ( X(1+cOff+layer.numPatch*2:cOff+layer.numPatch*3,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X4g = vertcat ( X(1+cOff+layer.numPatch*3:cOff+layer.numPatch*4,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X5g = vertcat ( X(1+cOff+layer.numPatch*4:cOff+layer.numPatch*5,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X6g = vertcat ( X(1+cOff+layer.numPatch*5:cOff+layer.numPatch*6,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X7g = vertcat ( X(1+cOff+layer.numPatch*6:cOff+layer.numPatch*7,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X8g = vertcat ( X(1+cOff+layer.numPatch*7:cOff+layer.numPatch*8,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X9g = vertcat ( X(1+cOff+layer.numPatch*8:cOff+layer.numPatch*9,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X10g = vertcat ( X(1+cOff+layer.numPatch*9:cOff+layer.numPatch*10,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X11g = vertcat ( X(1+cOff+layer.numPatch*10:cOff+layer.numPatch*11,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X12g = vertcat ( X(1+cOff+layer.numPatch*11:cOff+layer.numPatch*12,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X13g = vertcat ( X(1+cOff+layer.numPatch*12:cOff+layer.numPatch*13,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X14g = vertcat ( X(1+cOff+layer.numPatch*13:cOff+layer.numPatch*14,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X15g = vertcat ( X(1+cOff+layer.numPatch*14:cOff+layer.numPatch*15,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X16g = vertcat ( X(1+cOff+layer.numPatch*15:cOff+layer.numPatch*16,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X17g = vertcat ( X(1+cOff+layer.numPatch*16:cOff+layer.numPatch*17,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X18g = vertcat ( X(1+cOff+layer.numPatch*17:cOff+layer.numPatch*18,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X19g = vertcat ( X(1+cOff+layer.numPatch*18:cOff+layer.numPatch*19,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X20g = vertcat ( X(1+cOff+layer.numPatch*19:cOff+layer.numPatch*20,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X21g = vertcat ( X(1+cOff+layer.numPatch*20:cOff+layer.numPatch*21,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X22g = vertcat ( X(1+cOff+layer.numPatch*21:cOff+layer.numPatch*22,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X23g = vertcat ( X(1+cOff+layer.numPatch*22:cOff+layer.numPatch*23,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X24g = vertcat ( X(1+cOff+layer.numPatch*23:cOff+layer.numPatch*24,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X25g = vertcat ( X(1+cOff+layer.numPatch*24:cOff+layer.numPatch*25,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X1b = vertcat ( X(1+cOff:cOff+layer.numPatch,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X2b = vertcat ( X(1+cOff+layer.numPatch:cOff+layer.numPatch*2,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X3b = vertcat ( X(1+cOff+layer.numPatch*2:cOff+layer.numPatch*3,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X4b = vertcat ( X(1+cOff+layer.numPatch*3:cOff+layer.numPatch*4,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X5b = vertcat ( X(1+cOff+layer.numPatch*4:cOff+layer.numPatch*5,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X6b = vertcat ( X(1+cOff+layer.numPatch*5:cOff+layer.numPatch*6,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X7b = vertcat ( X(1+cOff+layer.numPatch*6:cOff+layer.numPatch*7,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X8b = vertcat ( X(1+cOff+layer.numPatch*7:cOff+layer.numPatch*8,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X9b = vertcat ( X(1+cOff+layer.numPatch*8:cOff+layer.numPatch*9,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X10b = vertcat ( X(1+cOff+layer.numPatch*9:cOff+layer.numPatch*10,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X11b = vertcat ( X(1+cOff+layer.numPatch*10:cOff+layer.numPatch*11,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X12b = vertcat ( X(1+cOff+layer.numPatch*11:cOff+layer.numPatch*12,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X13b = vertcat ( X(1+cOff+layer.numPatch*12:cOff+layer.numPatch*13,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X14b = vertcat ( X(1+cOff+layer.numPatch*13:cOff+layer.numPatch*14,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X15b = vertcat ( X(1+cOff+layer.numPatch*14:cOff+layer.numPatch*15,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X16b = vertcat ( X(1+cOff+layer.numPatch*15:cOff+layer.numPatch*16,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X17b = vertcat ( X(1+cOff+layer.numPatch*16:cOff+layer.numPatch*17,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X18b = vertcat ( X(1+cOff+layer.numPatch*17:cOff+layer.numPatch*18,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X19b = vertcat ( X(1+cOff+layer.numPatch*18:cOff+layer.numPatch*19,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X20b = vertcat ( X(1+cOff+layer.numPatch*19:cOff+layer.numPatch*20,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X21b = vertcat ( X(1+cOff+layer.numPatch*20:cOff+layer.numPatch*21,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X22b = vertcat ( X(1+cOff+layer.numPatch*21:cOff+layer.numPatch*22,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X23b = vertcat ( X(1+cOff+layer.numPatch*22:cOff+layer.numPatch*23,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X24b = vertcat ( X(1+cOff+layer.numPatch*23:cOff+layer.numPatch*24,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
            ...
            X25b = vertcat ( X(1+cOff+layer.numPatch*24:cOff+layer.numPatch*25,:), X(layer.numInChannels-layer.numResChannels+1:layer.numInChannels,:) );
