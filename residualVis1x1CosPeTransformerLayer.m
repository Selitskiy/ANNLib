classdef residualVis1x1CosPeTransformerLayer < nnet.layer.Layer % & nnet.layer.Formattable (Optional)
    ...
    methods
        function layer = residualVis1x1CosPeTransformerLayer(numInChannels, numResChannels, name)
            ...
            layer.Wq = bound * (2. * rand([layer.numOutChannels-layer.numResChannels, layer.numInChannels-layer.numResChannels],'single') - 1.);
            layer.Wq0 = zeros([layer.numOutChannels-layer.numResChannels, 1]);

            layer.Wk = bound * (2. * rand([layer.numOutChannels-layer.numResChannels, layer.numInChannels-layer.numResChannels],'single') - 1.);
            layer.Wk0 = zeros([layer.numOutChannels-layer.numResChannels, 1]);
        ...
        function [Z] = predict(layer, X)
            ...
            X1 = X(1:layer.numInChannels-layer.numResChannels, :);

            K = tanh(layer.Wk * X1 + layer.Wk0);
            Q = tanh(layer.Wq * X1 + layer.Wq0);
