function generic_err_ae_graph2D(M, Em, Er, l_m, Y2, Sy2, l_y, l_sess, x_off, x_in, t_in, y_off, y_out, t_out, k_tob, t_sess, sess_off, offset, k_start, modelName, yLab)
    ...
    for i = 1:t_sess-sess_off
        ...
        j=1;
        y_st2 = min(Sy2(1,j,i));
        for j = 1:(t_in+1):k_tob
            for k = 0:t_in
            Myw = reshape(Y2(:, j + k*k_tob, i), [y_out, t_out])';
            %lp = plot(Sy(1,j,i):Sy(2,j,i), Myw(:,k)+1, 'm', 'LineWidth', 2);
            M3(Sy2(1,j + k*k_tob,i):Sy2(2,j + k*k_tob,i), y_off+1:y_off+y_out) = Myw;
            y_end2 = max(Sy2(2,j + k*k_tob,i));
            end
        end
