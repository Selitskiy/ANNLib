classdef BtransNet2D < BtransLayers2D & BaseNet2D & MLPInputNet2D

    properties
    end

    methods
        function net = BtransNet2D(x_off, x_in, t_in, y_off, y_out, t_out, ini_rate, max_epoch)

            net = net@BaseNet2D(x_off, x_in, t_in, y_off, y_out, t_out, ini_rate, max_epoch);
            net = net@MLPInputNet2D();
            net = net@BtransLayers2D();
