classdef dpBatchSquizeTransformerLayer < nnet.layer.Layer % & nnet.layer.Formattable (Optional)
    ...
    properties
        ...
        numInChannels
        numOutChannels
        thresh % threshold for softmax 0-1
    end
    ...
    methods
        function layer = dpBatchSquizeTransformerLayer(numInChannels, thresh, name)
            ...
            layer.thresh = thresh;
            layer.thLearn = thresh;

            % Initialize weight coefficients.
            bound = sqrt(6 / (layer.numInChannels + layer.numInChannels));
            
            layer.Wq = bound * (2. * rand([layer.numInChannels, layer.numInChannels],'single') - 1.);
            layer.Wq0 = zeros([layer.numInChannels, 1]);

            layer.Wk = bound * (2. * rand([layer.numInChannels, layer.numInChannels],'single') - 1.);
            layer.Wk0 = zeros([layer.numInChannels, 1]);
